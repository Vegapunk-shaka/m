...
async def MergeVideo(input_file: str, user_id: int, message: Message, format_: str, extra_options: str = None):
    """
    ...
    :param `extra_options`: Additional options for the ffmpeg command.
    ...
    if extra_options:
        file_generator_command = file_generator_command + extra_options.split()
...
async def mergeNow(cb: CallbackQuery, input_: str):
    # Ensure the input file exists before proceeding
    if not os.path.exists(input_):
        await cb.message.edit("Input file not found!")
        return

    await cb.message.edit("Starting video merge...")

    # Call the updated MergeVideo function with extra options
    merged_video_path = await MergeVideo(
        input_file=input_,
        user_id=cb.from_user.id,
        message=cb.message,
        format_="mkv",
        extra_options="-fflags +genpts -r 30 -vsync vfr"  # Example extra options
    )

    if merged_video_path:
        await cb.message.edit("Video merged successfully!")
        # Further processing, e.g., sending the merged video
    else:
